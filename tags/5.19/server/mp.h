...
#define SOFTFLOW_PREFIX	0x01
#define SOFTFLOW_START	0x02
#define SOFTFLOW_STOP	0x03
#define SOFTFLOW_RSTART	0x11	/* ^Q */
#define SOFTFLOW_RSTOP	0x13	/* ^S */
...
extern int psionfd, debug, gmtoffset, exiting, psion_alive, dowakeup,
           old_nfsc, masterfd, query_cache, background, series5, soft_flow,
	   no_flow;
...
// NFS Buffer size. The localhost should ask for/send this size of packets.
// On linux values smaller than 1024 are not accepted, and default to 4k.
//#define PBUFSIZE       8192
#define PBUFSIZE       1024

#define TIMEOUT       -54
// Simulated fs blocksize on the psion
...
/* mp_mount.c */
void mount_and_run __P((char *dir, char *dev, void (*proc)(), mynfs_fh *root_fh));
...
struct attrstat *nfsproc_getattr_2 __P((struct mynfs_fh *fh));
struct attrstat *nfsproc_setattr_2 __P((struct sattrargs *sa));
struct statfsres *nfsproc_statfs_2 __P((struct mynfs_fh *fh));
struct readdirres *nfsproc_readdir_2 __P((struct readdirargs *ra));
struct readlinkres *nfsproc_readlink_2 __P((struct mynfs_fh *fh));
